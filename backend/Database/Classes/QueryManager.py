...
class QueryManager:
    ...
    def leave_activity(self, person_id, activity_name):
        ...
        self.db.execute_change(query)
        self.remove_activity(query)
    

    # Person
    
    def create_user(self, person_name, password):
        query = f"""INSERT INTO "Person" (person_name, password) VALUES ('{person_name}', '{password}')"""
        return self.db.execute_change(query)
    ...
    def obtain_person_category(self, person_id):
        query = f"""SELECT category_id FROM "Person" WHERE person_id='{person_id}'"""
        return self.db.execute_change(query)
